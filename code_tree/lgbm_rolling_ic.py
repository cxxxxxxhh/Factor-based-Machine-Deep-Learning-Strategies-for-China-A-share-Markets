...
def LGBM_metric(index):

    def custom_ic_valid_LGBM(y_pred, dtrain):
        epsilon = 1e-7
        ...
        return "custom_ic_valid",  r, True
    
    return custom_ic_valid_LGBM

data_dir = "/home/cheam/IP_alpha/lgbm_ic_rolling"
f=open(data_dir + "/train.txt","w+")
data = pd.read_parquet('/home/cheam/IP_alpha/data/factor_all_std.parquet')
results_all_tunning = pd.DataFrame(columns=['test_start_date','learning_rate','max_depth',
                                            'num_leaves','min_data_in_leaf','feature_fraction','bagging_fraction','out_ic','in_ic','out_mse','in_mse'])
...
def objective(trial): #先定义很多超参数的范围，然后利用这些参数训练模型，得到test error最小的，假设我们是不知道test data的吧，只能输出cv的test error
    ...
    # unique_dates = train_data.index.get_level_values('Date').unique()
    n_splits = 10
    ...
    for i in range(n_splits-2):
        ...
        Dvalid_op = lgb.Dataset(val_odata.drop('AdjVWAP',axis=1),label=val_odata['AdjVWAP'])
        ...
        model = lgb.train(params=params, train_set=Dtrain_op,
                                        valid_sets=Dvalid_op,
                                        feval=LGBM_metric(val_odata.index),
                                        callbacks=[
        ...
        # out of sample
        tunning_y_test_outsample = model.predict(test_odata.drop('AdjVWAP',axis=1))
        ...
        tunning_y_test_insample = model.predict(train_odata.drop('AdjVWAP',axis=1))
    ...
    results_all_tunning.loc[len(results_all_tunning)] = [test_start_date,learning_rate,max_depth,
                                            num_leaves,min_data_in_leaf,feature_fraction,bagging_fraction
                                            ,tunning_mean_ic_outsample,tunning_mean_ic_insample,
                                            tunning_mean_mse_outsample,tunning_mean_mse_insample]
...
score_mse_in_sample=[]
result_all_train = pd.DataFrame(columns=['test_start_date','learning_rate','max_depth',
                                            'num_leaves','min_data_in_leaf','feature_fraction','bagging_fraction'
                                            ,'out_of_sample_ic','in_sample_ic','out_of_sample_mse','in_sample_mse'])
for i in range(len(year)-5): 
    ...
    if year[i][1] == 1:# 调参
        ...
        absolute_path = data_dir + "/lgbm.db"
        study = optuna.create_study(
        storage=f"sqlite:///{absolute_path}",
        study_name="lgbm_ic_" + train_start_date+'_' + test_start_date,
        direction=direction,
        load_if_exists=True,
        )
        ...
        # 开始优化
        study.optimize(objective, n_jobs=1, callbacks=[early_stopping], timeout=10800)
        best_params = study.best_params
        best_params.update(additional_params)
        print(best_params,file=f,flush=True)
    Dtrain = lgb.Dataset(train_data.drop('AdjVWAP',axis=1), label=train_data['AdjVWAP'])
    Dvalid = lgb.Dataset(valid_data.drop('AdjVWAP',axis=1), label=valid_data['AdjVWAP'])
    Dtest = lgb.Dataset(test_data.drop('AdjVWAP',axis=1), label=test_data['AdjVWAP'])
    #watchlist = [(Dtrain,'train'),(Dvalid, 'eval')]
    evals = {}
    model = lgb.train(params=best_params, train_set=Dtrain,
                                        valid_sets=Dvalid,
                                        feval=LGBM_metric(valid_data.index),
                                        callbacks=[
                                            lgb.early_stopping(stopping_rounds=10),
                                            lgb.log_evaluation(),
                                            lgb.record_evaluation(evals),
    ...
    model_name = data_dir + '/lgbm_ic_' + train_start_date+'_'+test_start_date+ variable_part + '.json'
    ...
    ## in sample
    y_pred_in = pd.DataFrame(model.predict(train_data.drop('AdjVWAP',axis=1)))
    ...
    # 保存结果
    result_all_train.loc[len(result_all_train)] = [test_start_date,best_params['learning_rate'],best_params['max_depth'],
                                                   best_params['num_leaves'],best_params['min_data_in_leaf'],
                                                    best_params['feature_fraction'],best_params['bagging_fraction'],
                                                    train_ic_outsample,train_ic_insample,train_mse_outsample,train_mse_insample]
    print("result_all_train",result_all_train,file=f,flush=True)
results['mean']=results.mean(axis=1)
results = results.sort_index(level='Date')
results.to_parquet(data_dir + '/results_lgbm.parquet')
results_insample['mean']=results_insample.mean(axis=1)
results_insample = results_insample.sort_index(level='Date')
results_insample.to_parquet(data_dir + '/results_insample_lgbm.parquet')
